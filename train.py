...
def validate(model, test_loader, writer, device, n_iter):
    ...
    for batch in test_loader:
        text_padded, input_lengths, mel_padded, gate_padded, \
            output_lengths, speaker_ids = batch_to_device(batch, device)
    ...
    # Logging (adjust for missing alignments)
    idx = torch.randint(0, mel_padded.size(0), (1,)).item()
    mel_infer, mel_lens, dur_pred = model.infer(
        text_padded[idx:idx+1], input_lengths[idx:idx+1], speaker_ids[idx:idx+1])

    # Since alignments aren’t available, log durations or skip alignment logging
    writer.add_sample(
        dur_pred[idx:idx+1],  # Use durations instead of alignments
...
def training_loop(model,
    ...
    for epoch in range(n_epoch, config.epochs):
        ...
        for batch in train_loader:
            ...
            # Compute loss (mel spectrogram loss only)
            mel_loss = F.mse_loss(mel_out, mel_padded) + F.mse_loss(mel_postnet_out, mel_padded)
            loss = mel_loss
            ...
            if n_iter % config.n_save_states_iter == 0:
                save_states(f'states.pth', model, optimizer, n_iter, epoch, config)

            if n_iter % config.n_save_backup_iter == 0 and n_iter > 0:
                save_states(f'states_{n_iter}.pth', model, optimizer, n_iter, epoch, config)
